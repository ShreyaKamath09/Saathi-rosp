    ...
    useEffect(() => {
        const getFlats = async () => {
            try {
                ...
                const deetsResponse = await databases.listDocuments(
                    "647beff6d2bb278e1166",
                    "6480bea54aea6eb0543b"
                );
                const flatsWithDetails = filesResponse.files
                    .map((file, index) => {
                        const document = deetsResponse.documents[index];
                        return document
                            ? {
                                  id: file.$id,
                                  file,
                        ...
                        {flats.map((flat) => (
                            <div
                                ...
                                onClick={() =>
                                    handleFlatClick(
                                        flat.id,
                                        flat.deet.documentId
                                    )
                                }
                            >
                                <img
                                    src={getFilePreview(flat.file)}
                                    alt={flat.file.name}
                                    className="flatsphoto"
                                />
                                <h2>Rs.{flat.deet.rent} per month</h2>
                                <p>
                                    <Hotel /> {flat.deet.bedsAvail} bhk
                                </p>
                                <p>{flat.deet.city} </p>
